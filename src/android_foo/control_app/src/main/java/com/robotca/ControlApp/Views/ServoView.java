 ...
 *         Nathaniel Stone
 */
public class ServoView extends RelativeLayout implements AnimationListener,
        MessageListener<nav_msgs.Odometry>/*, NodeMain*/ {

    /**
     * TAG Debug Log tag.
     */
    private static final String TAG = "ServoView";
    ...
    private final SensorEventListener ACCEL_LISTENER = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event)
        {
            ...
            Display display = ((WindowManager) ServoView.this.getContext().getSystemService(
    ...
    public ServoView(Context context, AttributeSet attrs, int defStyle) {
...
//        thumbDivet.setImageResource(R.drawable.button);
    ...
    private void onContactMove(float x, float y) {
        ...
        // Publish the servo.
        if (holonomic) {
            publishServo(normalizedMagnitude * Math.cos(contactTheta * Math.PI / 180.0),
...
//        }
     ...
     *
     * @param pitch  The normalized linear velocity (-1 to 1).
     * @param yaw The normalized angular velocity (-1 to 1).
     */
    private void publishServo(double pitch, double yaw ) {

        ((ControlApp) getContext()).getRobotController().
                forceServo(pitch, yaw);
    }
